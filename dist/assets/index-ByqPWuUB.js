 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const RI=Math.log(2);class PI{constructor(e){const n=s=>parseInt(Math.log(s)/RI,10),r=s=>parseInt(Array(s+1).join("1"),2);this.count=n(e+1),this.current_=this.count-1;const i=r(this.count);this.bits_=e+1&i}nextBitIsOne(){const e=!(this.bits_&1<<this.current_);return this.current_--,e}}const dl=function(t,e,n,r){t.sort(e);const i=function(a,u){const d=u-a;let c,h;if(d===0)return null;if(d===1)return c=t[a],h=n?n(c):c,new we(h,c.node,we.BLACK,null,null);{const g=parseInt(d/2,10)+a,_=i(a,g),w=i(g+1,u);return c=t[g],h=n?n(c):c,new we(h,c.node,we.BLACK,_,w)}},s=function(a){let u=null,d=null,c=t.length;const h=function(_,w){const O=c-_,p=c;c-=_;const f=i(O+1,p),m=t[O],E=n?n(m):m;g(new we(E,m.node,w,null,f))},g=function(_){u?(u.left=_,u=_):(d=_,u=_)};for(let _=0;_<a.count;++_){const w=a.nextBitIsOne(),O=Math.pow(2,a.count-(_+1));w?h(O,we.BLACK):(h(O,we.BLACK),h(O,we.RED))}return d},o=new PI(t.length),l=s(o);return new Be(r||e,l)};/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
${_e}
${window.location.href}`;navigator.clipboard.writeText(nn).then(()=>{eh("Sonuç panoya kopyalandı")})}function ev(){const y=xo(Wn);if(y.length!==V){K(`${V} harfli bir kelime gir.`);return}const P=qN(y),B=`${window.location.origin}${window.location.pathname}?challenge=${P}`;navigator.clipboard.writeText(B).then(()=>{eh("Meydan okuma linki panoya kopyalandı"),u(!1)})}function tv(){X([]),Se(""),$n(0),c(!1),g([]),en.current=!1,K(`Yeni oyun basladi. ${V} harfli kelimeyi bul.`)}function nv(y){xe||k||js(y)}function rv(){s(y=>Math.min(Ip,Number((y+Tp).toFixed(2))))}function iv(){s(y=>Math.max(kp,Number((y-Tp).toFixed(2))))}function sv(){Zt(!0),X([]),Se(""),$n(0),en.current=!1,K(`${V} harfli kelimeyi bul.`)}async function ov(){if(!Ot){K("Canlı oda için Firebase ayarları eksik.");return}if(!O.trim()){K("Önce adını gir.");return}const y=xo(x),P=new RegExp(`^[A-ZÇĞİIÖŞÜ]{${V}}$`,"u");if(E){if(y.length!==V){K(`Admin kelimesi ${V} harf olmalı.`);return}if(!P.test(y)){K("Admin kelimesi sadece harf içermeli.");return}}try{const B=E?y:A,_e=await BN(V,B,O.trim());te({code:_e.code,uid:_e.uid,hostId:_e.hostId,wordLength:V,targetWord:B}),X([]),Se(""),$n(0),c(!1),g([]),en.current=!1,m(_e.code),Zt(!0),K(`Canlı oda açıldı: ${_e.code}`)}catch{K("Canlı oda oluşturulamadı.")}}async function lv(){if(!Ot){K("Canlı oda için Firebase ayarları eksik.");return}if(!O.trim()){K("Önce adını gir.");return}const y=f.trim().toUpperCase();if(y.length<4){K("Geçerli oda kodu gir.");return}try{const P=await VN(y,O.trim());if(!P){K("Oda bulunamadı.");return}te({code:P.code,uid:P.uid,hostId:P.hostId,wordLength:P.wordLength,targetWord:P.targetWord}),js(P.wordLength),X([]),Se(""),$n(0),c(!1),g([]),en.current=!1,Zt(!0),K(`Canlı odaya katıldın: ${P.code}`)}catch{K("Odaya katılınamadı.")}}function av(){if(xe){window.location.href=`${window.location.origin}${window.location.pathname}`;return}Zt(!1),u(!1),X([]),Se(""),$n(0),te(null),je([]),R(""),T(!1),c(!1),g([]),en.current=!1,K(`${V} harfli kelimeyi bul.`)}const ta=`${String(Math.floor(Ws/60)).padStart(2,"0")}:${String(Ws%60).padStart(2,"0")}`,uv=n.played?Math.round(n.wins/n.played*100):0;return v.jsxs("main",{className:"mx-auto flex min-h-screen w-full max-w-md flex-col gap-4 bg-[hsl(var(--bg))] px-3 pb-[max(env(safe-area-inset-bottom),12px)] pt-4 text-[hsl(var(--text))] sm:px-4",children:[Yd&&v.jsx("div",{className:"pointer-events-none fixed left-1/2 top-1/2 z-40 -translate-x-1/2 -translate-y-1/2",children:v.jsx("div",{className:"rounded-2xl border border-emerald-400/40 bg-zinc-900/95 px-6 py-3 text-base font-semibold text-emerald-300 shadow-2xl sm:text-lg",children:Yd})}),h.length>0&&v.jsx("div",{className:"pointer-events-none fixed inset-0 z-30 overflow-hidden",children:h.map(y=>v.jsx("span",{className:"confetti-piece",style:{left:`${y.left}%`,width:`${y.size}px`,height:`${y.size*1.6}px`,backgroundColor:y.color,animationDelay:`${y.delay}s`,animationDuration:`${y.duration}s`,transform:`rotate(${y.rotate}deg)`}},y.id))}),v.jsxs("header",{className:"-mx-3 bg-black px-3 py-3 sm:-mx-4 sm:px-4",children:[v.jsxs("div",{className:"mx-auto flex w-full max-w-md items-center justify-between gap-2",children:[ct?v.jsx("button",{type:"button",onClick:av,className:"rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs font-semibold text-zinc-100",children:"← Geri"}):v.jsx("button",{type:"button",onClick:()=>l(!0),className:"rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs font-semibold text-zinc-100",children:"İstatistik"}),v.jsxs("h1",{className:"flex items-center gap-2 text-xl font-bold sm:text-2xl",children:[v.jsx("img",{src:"/logo.svg",alt:"Kelime Oyunu logosu",className:"h-7 w-7 rounded-md"}),"KELİME OYUNU",v.jsx("img",{src:"/logo.svg",alt:"Kelime Oyunu logosu",className:"h-7 w-7 rounded-md"})]}),v.jsx("button",{type:"button",onClick:()=>e(y=>y==="dark"?"light":"dark"),className:"rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs font-semibold text-zinc-100",children:t==="dark"?"Light":"Dark"})]}),v.jsx("p",{className:"mx-auto mt-1 w-full max-w-md text-center text-xs text-zinc-400",children:k?`Canlı Oda: ${k.code}`:xe?"Arkadaş meydan okuması":"Günün Kelimesi Modu"})]}),!ct&&!xe&&v.jsxs(v.Fragment,{children:[v.jsxs("section",{className:"rounded-2xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-5 shadow-sm sm:p-6",children:[v.jsx("h2",{className:"mb-2 text-center text-xl font-bold sm:text-2xl",children:"Oyun Ayarı"}),v.jsx("p",{className:"mb-4 text-center text-sm text-[hsl(var(--muted))] sm:text-base",children:"Harf sayısını seç, sonra oyuna başla!"}),v.jsx("div",{className:"mb-4 flex justify-center gap-2",children:lc.map(y=>v.jsxs("button",{type:"button",onClick:()=>nv(y),className:`rounded-lg px-4 py-2 text-sm font-semibold sm:text-base ${V===y?"bg-cyan-700 text-white":"bg-[hsl(var(--surface2))] text-[hsl(var(--text))]"}`,children:[y," Harf"]},y))}),v.jsx("button",{type:"button",onClick:sv,className:"w-full rounded-lg bg-emerald-600 px-4 py-3 text-base font-semibold text-white sm:py-4 sm:text-lg",children:"Günün Kelimesi"}),v.jsx("button",{type:"button",onClick:()=>u(!0),className:"mt-3 w-full rounded-lg bg-cyan-700 px-4 py-3 text-base font-semibold text-white sm:py-4 sm:text-lg",children:"Arkadaşına Meydan Oku"})]}),v.jsxs("section",{className:"rounded-2xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-5 shadow-sm sm:p-6",children:[v.jsx("h3",{className:"mb-2 text-center text-lg font-bold sm:text-xl",children:"Canlı Oda (2-10 Kişi)"}),v.jsx("p",{className:"mb-3 text-center text-sm text-[hsl(var(--muted))]",children:"Adını gir, oda oluştur veya kodla katıl."}),v.jsx("input",{value:O,onChange:y=>p(y.target.value),placeholder:"Adın",className:"mb-2 w-full rounded-lg border border-[hsl(var(--stroke))] bg-[hsl(var(--surface2))] px-3 py-2 text-sm outline-none"}),v.jsxs("label",{className:"mb-2 flex items-center gap-2 text-xs text-[hsl(var(--muted))]",children:[v.jsx("input",{type:"checkbox",checked:E,onChange:y=>T(y.target.checked)}),"Kelimeyi sen belirle (Oda sahibi / Admin)"]}),E&&v.jsx("input",{value:x,onChange:y=>R(y.target.value),maxLength:V,placeholder:`Admin kelimesi (${V} harf)`,className:"mb-2 w-full rounded-lg border border-[hsl(var(--stroke))] bg-[hsl(var(--surface2))] px-3 py-2 text-sm uppercase outline-none"}),v.jsxs("div",{className:"grid grid-cols-2 gap-2",children:[v.jsx("button",{type:"button",onClick:ov,disabled:!O.trim(),className:"rounded-lg bg-violet-600 px-3 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50",children:"Oda Oluştur"}),v.jsx("button",{type:"button",onClick:lv,disabled:!O.trim()||f.trim().length<4,className:"rounded-lg bg-sky-700 px-3 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50",children:"Odaya Katıl"})]}),v.jsx("input",{value:f,onChange:y=>m(y.target.value.toUpperCase()),placeholder:"Oda Kodu (ör. A1B2C3)",className:"mt-2 w-full rounded-lg border border-[hsl(var(--stroke))] bg-[hsl(var(--surface2))] px-3 py-2 text-sm uppercase outline-none"}),!Ot&&v.jsx("p",{className:"mt-2 text-xs text-amber-400",children:"Firebase ENV tanımlanınca canlı oda aktif olur."})]})]}),ct&&v.jsxs(v.Fragment,{children:[v.jsxs("section",{className:"rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-3 shadow-sm",children:[v.jsxs("div",{className:"mb-2 flex items-center justify-between",children:[v.jsxs("span",{className:"flex items-center gap-1 rounded-full border border-cyan-500/40 bg-cyan-950/60 px-3 py-1 text-xs font-semibold text-cyan-300",children:[v.jsx("span",{"aria-hidden":"true",children:"⏳"}),ta]}),v.jsx("span",{className:"text-center text-sm text-emerald-500",children:pi}),v.jsx("span",{className:"w-[62px]"})]}),v.jsx("div",{className:"grid grid-rows-6 gap-2",children:qy.map((y,P)=>{const B=y.length===V&&P<U.length?Va(y,A):[];return v.jsx("div",{className:"grid gap-1.5",style:{gridTemplateColumns:`repeat(${V}, minmax(0, 1fr))`},children:Array.from({length:V}).map((_e,Dt)=>{const gr=y[Dt]??"",nn=B[Dt]??"unknown",Bn=nn==="correct"?"bg-green-600 border-green-500":nn==="present"?"bg-yellow-600 border-yellow-500":nn==="absent"?"bg-zinc-700 border-zinc-600":"border-[hsl(var(--stroke))] bg-[hsl(var(--surface2))]";return v.jsx("div",{className:`flex aspect-square items-center justify-center rounded border text-lg font-bold uppercase sm:text-xl ${Bn}`,children:gr},`${P}-${Dt}`)})},P)})})]}),k&&v.jsxs("section",{className:"rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-3",children:[v.jsxs("div",{className:"mb-2 flex items-center justify-between",children:[v.jsxs("h3",{className:"text-sm font-bold",children:["Canlı Oda: ",k.code]}),v.jsxs("span",{className:"text-xs text-[hsl(var(--muted))]",children:[F.length," oyuncu"]})]}),v.jsx("div",{className:"space-y-1",children:F.map(y=>v.jsxs("div",{className:"flex items-center justify-between rounded bg-[hsl(var(--surface2))] px-2 py-1 text-xs",children:[v.jsxs("span",{children:[y.name||"Oyuncu",y.id===k.hostId?" (Admin)":""]}),v.jsx("span",{className:"text-[hsl(var(--muted))]",children:y.solved?`${y.attempts}. Deneme • ${Math.floor(y.elapsedSeconds/60)}:${String(y.elapsedSeconds%60).padStart(2,"0")}`:y.attempts>=et?`${et}. Deneme • Bilemedi`:`${y.attempts}. Deneme • Çözmedi`})]},y.id))})]}),v.jsxs("section",{className:"sticky bottom-0 z-10 space-y-2 rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))]/95 p-3 backdrop-blur",children:[v.jsxs("div",{className:"mb-1 flex items-center justify-between text-xs",children:[v.jsx("span",{className:"text-[hsl(var(--muted))]",children:"Klavye Boyutu"}),v.jsxs("div",{className:"flex items-center gap-1",children:[v.jsx("button",{type:"button",onClick:iv,className:"rounded bg-[hsl(var(--surface2))] px-2 py-1 font-semibold",children:"-"}),v.jsxs("span",{className:"w-10 text-center",children:[Math.round(i*100),"%"]}),v.jsx("button",{type:"button",onClick:rv,className:"rounded bg-[hsl(var(--surface2))] px-2 py-1 font-semibold",children:"+"})]})]}),KN.map((y,P)=>v.jsx("div",{className:"flex justify-center gap-1",children:y.map(B=>{const _e=Xy.get(B)??"unknown",Dt=_e==="correct"?"bg-green-600 text-white":_e==="present"?"bg-yellow-600 text-white":_e==="absent"?"bg-zinc-800 text-zinc-400":"bg-zinc-500 text-white";return v.jsx("button",{type:"button",onClick:()=>Jy(B),style:{minHeight:`${48*i}px`,minWidth:`${(B==="ENTER"||B==="SİL"?62:32)*i}px`,fontSize:`${13*i}px`,paddingInline:`${9*i}px`,paddingBlock:`${10*i}px`},className:`rounded font-semibold transition duration-100 active:scale-95 active:brightness-75 ${Dt}`,children:B},B)})},P))]}),v.jsx("section",{className:"grid gap-2 rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-3",children:v.jsxs("div",{className:"grid grid-cols-2 gap-2",children:[v.jsx("button",{type:"button",onClick:Zy,className:"rounded bg-emerald-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50",disabled:!Ct,children:"Sonucu Paylaş"}),v.jsx("button",{type:"button",onClick:tv,className:"rounded bg-indigo-600 px-4 py-3 text-sm font-semibold text-white",children:"Yeni Oyun"})]})})]}),o&&v.jsx("div",{className:"fixed inset-0 z-20 flex items-center justify-center bg-black/60 p-4",children:v.jsxs("div",{className:"w-full max-w-sm rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-4",children:[v.jsxs("div",{className:"mb-3 flex items-center justify-between",children:[v.jsx("h2",{className:"text-lg font-bold",children:"İstatistikler"}),v.jsx("button",{type:"button",onClick:()=>l(!1),className:"rounded bg-[hsl(var(--surface2))] px-3 py-1 text-sm",children:"Kapat"})]}),v.jsxs("div",{className:"grid grid-cols-2 gap-2 text-sm",children:[v.jsxs("div",{className:"rounded bg-[hsl(var(--surface2))] p-2",children:["Oyun: ",n.played]}),v.jsxs("div",{className:"rounded bg-[hsl(var(--surface2))] p-2",children:["Kazanma: %",uv]}),v.jsxs("div",{className:"rounded bg-[hsl(var(--surface2))] p-2",children:["Seri: ",n.currentStreak]}),v.jsxs("div",{className:"rounded bg-[hsl(var(--surface2))] p-2",children:["En iyi: ",n.bestStreak]})]}),v.jsx("div",{className:"mt-3 space-y-1 text-xs",children:n.distribution.map((y,P)=>v.jsxs("div",{className:"flex items-center gap-2",children:[v.jsx("span",{className:"w-4",children:P+1}),v.jsx("div",{className:"h-4 flex-1 rounded bg-[hsl(var(--surface2))]",children:v.jsx("div",{className:"h-4 rounded bg-emerald-600",style:{width:`${Math.min(100,y*15)}%`}})}),v.jsx("span",{className:"w-6 text-right",children:y})]},P))})]})}),a&&v.jsx("div",{className:"fixed inset-0 z-20 flex items-center justify-center bg-black/60 p-4",children:v.jsxs("div",{className:"w-full max-w-sm rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-4",children:[v.jsxs("div",{className:"mb-3 flex items-center justify-between",children:[v.jsx("h2",{className:"text-lg font-bold",children:"Arkadaşına Meydan Oku"}),v.jsx("button",{type:"button",onClick:()=>u(!1),className:"rounded bg-[hsl(var(--surface2))] px-3 py-1 text-sm",children:"Kapat"})]}),v.jsxs("p",{className:"mb-3 text-xs text-[hsl(var(--muted))]",children:[V," harfli kelime gir. Link oluşunca arkadaşına gönder."]}),v.jsxs("div",{className:"flex gap-2",children:[v.jsx("input",{value:Wn,onChange:y=>Yy(y.target.value),maxLength:V,placeholder:`${V} harfli kelime`,className:"w-full rounded border border-[hsl(var(--stroke))] bg-[hsl(var(--surface2))] px-3 py-2 text-sm outline-none"}),v.jsx("button",{type:"button",onClick:ev,className:"rounded bg-cyan-700 px-4 py-2 text-sm font-semibold text-white",children:"Link Üret"})]})]})}),d&&v.jsx("div",{className:"fixed inset-0 z-30 flex items-center justify-center bg-black/70 p-4",children:v.jsxs("div",{className:"w-full max-w-sm rounded-xl border border-[hsl(var(--stroke))] bg-[hsl(var(--surface))] p-4 text-center",children:[v.jsx("h2",{className:"mb-2 text-2xl font-extrabold text-rose-400",children:"BİLEMEDİN!"}),v.jsxs("p",{className:"mb-3 text-sm text-[hsl(var(--muted))]",children:["Kelime: ",A]}),!_&&v.jsx("img",{src:"/bilemedin.png",alt:"Bilemedin",onError:()=>w(!0),className:"mx-auto mb-3 max-h-56 w-auto rounded-lg border border-[hsl(var(--stroke))]"}),v.jsx("button",{type:"button",onClick:()=>c(!1),className:"rounded bg-indigo-600 px-4 py-2 text-sm font-semibold text-white",children:"Tamam"})]})}),v.jsxs("footer",{className:"pb-2 text-center text-[11px] text-zinc-500",children:[v.jsx("p",{children:"Bağımsız Kelime Oyunu"}),v.jsx("p",{className:"text-[10px] opacity-90",children:"Contact: zumerrgurlenn@gmail.com"})]})]})}WE({immediate:!0});Ha.createRoot(document.getElementById("root")).render(v.jsx(Tv.StrictMode,{children:v.jsx(JN,{})}));
